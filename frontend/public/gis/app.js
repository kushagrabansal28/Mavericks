...
const GridApp = (() => {
  ...
  async function init() {
    ...
    window.addEventListener("message", (event) => {
      ...
      if (event.data?.type === "gridsense-focus-asset" && event.data.assetId) focusRealtimeTransformer(event.data.assetId);
    });
  ...
  function populateConditionMenu(conditions) {
    ...
    if (!selector) return;
    const options = document.createDocumentFragment();
    const prompt = new Option(`Select a GNN condition (${conditions.length.toLocaleString()} indexed - search to narrow)`, "");
    prompt.selected = true;
    options.appendChild(prompt);
    conditions.slice(0, 250).forEach((condition) => {
  ...
  /** Move the map to the first grid asset located in the chosen city. */
  function selectCity(city) {
    if (!city || !state.map) return;

    const datasetCity = state.citiesById[city];
    if (datasetCity) {
      showDatasetCity(datasetCity);
      return;
    }
  ...
  async function loadDatasetCondition(condition) {
    ...
    try {
      ...
      renderGnnAssessment(await response.json());
    } catch (error) {
