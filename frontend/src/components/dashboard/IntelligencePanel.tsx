...
export function IntelligencePanel() {
  ...
  const actionLabel = (prediction?.cascade_risk_pct ?? 0) >= 40 ? "Repair and redistribute" : "Keep operating with monitoring";
      ...
      <div className="p-3 space-y-2.5">
        ...
        {hasStress && rootCause && (
          <div className="panel p-3 border border-grid-cyan/30">
            ...
            <div className="mb-2 flex items-center justify-between rounded border border-grid-border/60 bg-grid-bg-primary px-2.5 py-2"><span className="label text-[9px]">Operator decision</span><span className={`font-mono text-[10px] font-bold ${actionLabel.startsWith("Repair") ? "text-grid-yellow" : "text-grid-green"}`}>{actionLabel}</span></div>
            {!isMitigated ? (
              <>
