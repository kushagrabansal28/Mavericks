...
export default function IntelligencePage() {
  const { prediction, gridState, setGridState, setPrediction } = useGridStore();
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        ...
        if (active) { setGridState(live.grid_state); setPrediction(live.model_output); setUpdatedAt(live.last_updated); }
      } catch {
        const s = DEMO_SCENARIOS.cascade_visualized();
        if (active) { setGridState(s.gridState); setPrediction(s.prediction); }
      }
    };
    void refresh();
      ...
      <main className="pt-16 min-h-screen bg-gs-gray-50">
        ...
        <section className="bg-white border-b border-gs-gray-200">
          <div className="container-official py-10">
            ...
            <p className="text-gs-gray-600 mt-2">Live model predictions, root-cause evidence and cascade forecasting. {updatedAt ? `Last model refresh: ${new Intl.DateTimeFormat("en-IN", { timeZone: "Asia/Kolkata", hour: "2-digit", minute: "2-digit", second: "2-digit" }).format(new Date(updatedAt))} IST.` : "Connecting to the model."}</p>
          </div>
        </section>
        ...
        <div className="container-official py-10 space-y-6">
          ...
          <div className="grid md:grid-cols-4 gap-4">
            ...
            <SummaryCard icon={<AlertOctagon className="w-5 h-5" />} color="red" label="Critical Assets" value={gridState?.nodes.filter((node) => node.status === "critical" || node.status === "root_cause").length ?? 0} />
                ...
                {nodeRisks.map(([id, risk]) => {
                  ...
                  return (
                    <tr key={id} className="border-b border-gs-gray-100 hover:bg-gs-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <Link href={`/map-explorer?asset=${encodeURIComponent(id)}`} className="text-xs text-gs-blue-600 hover:underline">View →</Link>
                      </td>
                    </tr>
                  );
                })}
