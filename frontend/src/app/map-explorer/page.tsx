...
export default function MapExplorerPage() {
  ...
  const [requestedAsset] = useState<string | null>(() => typeof window === "undefined" ? null : new URLSearchParams(window.location.search).get("asset"));
  ...
  useEffect(() => {
    if (viewMode !== "gis" || !requestedAsset) return;
    const focusAsset = () => gisFrame.current?.contentWindow?.postMessage({ type: "gridsense-focus-asset", assetId: requestedAsset }, window.location.origin);
    const timer = window.setTimeout(focusAsset, 300);
    return () => window.clearTimeout(timer);
  }, [requestedAsset, viewMode]);

  return (
    <>
      <Navbar variant="dark" />
      <LiveTelemetryUpdater />
      <main className="gs-workspace pt-16 h-screen w-screen flex flex-col overflow-hidden">
          ...
          <div className="flex-1 w-full h-full relative bg-gs-bg-primary overflow-hidden">
            <iframe
              ...
              onLoad={() => {
                const theme = document.documentElement.dataset.theme === "light" ? "light" : "dark";
                gisFrame.current?.contentWindow?.postMessage({ type: "gridsense-theme", theme }, window.location.origin);
                if (requestedAsset) gisFrame.current?.contentWindow?.postMessage({ type: "gridsense-focus-asset", assetId: requestedAsset }, window.location.origin);
              }}
