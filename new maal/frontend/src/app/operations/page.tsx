...
export default function OperationsPage() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setAssets(telemetry.transformers); setScenarios(scenarioData); setUpdatedAt(new Date(telemetry.timestamp).toLocaleTimeString());
  ...
  const exportBrief = () => {
    const content = ["GridSense operational brief", `Generated: ${new Date().toLocaleString()}`, "", ...alerts.map((a) => `${a.workflow_state} | ${a.asset_id} | ${a.telemetry.city}, ${a.telemetry.state} | risk ${a.telemetry.risk_pct}% | ${a.telemetry.status}`)].join("\n");
