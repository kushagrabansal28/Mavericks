...
const istTime = () => new Intl.DateTimeFormat("en-IN", { timeZone: "Asia/Kolkata", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: true }).format(new Date());
...
export function StatsSection() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!active) return;
      setTelemetry({ grid, online: health.status === "ok" || health.status === "healthy", updatedAt: istTime() });
    };
    void load();
  ...
  const metrics = useMemo<Metric[]>(() => {
    ...
    return [
      ...
      { value: telemetry.online ? "LIVE" : "SYNC", suffix: "", label: "Intelligence core", detail: telemetry.updatedAt ? `Updated ${telemetry.updatedAt} IST` : "Connecting to model", icon: Cpu, online: telemetry.online },
    ];
  }, [telemetry]);
