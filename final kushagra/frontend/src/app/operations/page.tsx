...
const formatIst = (value: string) => new Intl.DateTimeFormat("en-IN", { timeZone: "Asia/Kolkata", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: true }).format(new Date(value));
...
export default function OperationsPage() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setAssets(telemetry.transformers); setScenarios(scenarioData); setUpdatedAt(formatIst(telemetry.timestamp));
  ...
  const exportBrief = () => {
    const content = ["GridSense operational brief", `Generated (IST): ${formatIst(new Date().toISOString())}`, "", ...alerts.map((a) => `${a.workflow_state} | ${a.asset_id} | ${a.telemetry.city}, ${a.telemetry.state} | risk ${a.telemetry.risk_pct}% | ${a.telemetry.status}`)].join("\n");
  ...
  return <><Navbar /><main className="min-h-screen bg-gs-gray-50 pt-24 pb-14"><div className="container-official">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.55fr_1fr]">
          <section className="rounded-xl border border-gs-gray-200 bg-white overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-gs-gray-200 p-5"><div><h2 className="font-bold text-gs-gray-900">Live asset queue</h2><p className="text-sm text-gs-gray-500">Updated {updatedAt ? `${updatedAt} IST` : "..."}; refreshes every 30 seconds.</p></div><label className="flex items-center gap-2 rounded border border-gs-gray-300 px-3 py-2 text-sm"><Search className="h-4 w-4 text-gs-gray-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="City or asset" className="w-36 outline-none" /></label></div><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-gs-gray-50 text-xs uppercase tracking-wide text-gs-gray-500"><tr><th className="p-4">Asset</th><th className="p-4">Condition</th><th className="p-4">Risk</th><th className="p-4">Workflow</th></tr></thead><tbody>{visibleAssets.map((asset) => { const alert = alerts.find((item) => item.asset_id === asset.asset_id); return <tr key={asset.asset_id} className="border-t border-gs-gray-100 hover:bg-gs-gray-50"><td className="p-4"><Link href={`/map-explorer?asset=${encodeURIComponent(asset.asset_id)}`} className="font-mono font-bold text-gs-blue-600 hover:underline">{asset.asset_id}</Link><p className="text-xs text-gs-gray-500">{asset.city}, {asset.state} · {asset.voltage_kv} kV</p></td><td className="p-4"><p>{asset.load_pct}% load · {asset.temperature_c}°C</p><p className="text-xs text-gs-gray-500">{asset.voltage_pu} pu</p></td><td className="p-4"><Risk risk={asset.risk_pct} status={asset.status} /></td><td className="p-4">{alert ? <div className="flex flex-wrap gap-1"><span className="rounded bg-gs-gray-100 px-2 py-1 text-xs font-semibold">{alert.workflow_state}</span>{alert.workflow_state === "New" && <button onClick={() => void transition(asset.asset_id, "Acknowledged")} className="text-xs text-gs-blue-600">Acknowledge</button>}{alert.workflow_state === "Acknowledged" && <button onClick={() => void transition(asset.asset_id, "Assigned")} className="text-xs text-gs-blue-600">Assign</button>}{alert.workflow_state === "Assigned" && <button onClick={() => void transition(asset.asset_id, "Resolved")} className="text-xs text-green-700">Resolve</button>}</div> : <span className="text-xs text-green-700">No active alert</span>}</td></tr>; })}</tbody></table></div></section>
