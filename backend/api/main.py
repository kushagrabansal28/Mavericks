...
# Dataset Paths.  The original prototype expected ``backend/dataset`` while
# the repository stores the supported national datasets under ``backend/ml``.
# Resolve the real location first, retaining the legacy path only for older
# deployments that still use it.
ML_DATASET_DIR = os.path.join(ROOT_DIR, "ml", "datasets")
LEGACY_DATASET_DIR = os.path.join(ROOT_DIR, "dataset")
DATASET_DIR = ML_DATASET_DIR if os.path.isdir(ML_DATASET_DIR) else LEGACY_DATASET_DIR
...
def list_scenarios(limit: int = Query(5000, ge=1, le=5000)):
    """Returns pre-calibrated pitch scenarios and dataset scenarios."""
    # Keep the endpoint usable in worker/test environments where FastAPI's
    # lifespan hook has not yet run.
    ...
    if CANNED_SCENARIOS:
        for s_id, s_data in CANNED_SCENARIOS.items():
            scenarios_summary.append({
                ...
                "pitch_step": s_data.get("pitch_step", 1)
            })
...
def assess_dataset_condition(scenario_id: str):
    ...
    calibrated_risk_pct = round(0.70 * float(scenario.get("cascade_risk_pct") or 0) + 0.30 * raw_model_risk_pct, 1)
    ...
    return {
        ...
        "gnn_assessment": {
            ...
            "cascade_risk_pct": calibrated_risk_pct,
            "top_risk_nodes": [{"node_id": scenario.get("initiating_node"), "risk": round(calibrated_risk_pct / 100.0, 3), "time_to_critical_hours": None}] + [{"node_id": node_id, "risk": risk, "time_to_critical_hours": gnn_output["time_to_critical"].get(node_id)} for node_id, risk in top_risks]
        }
    }
...
def get_realtime_transformers(limit: int = Query(12, ge=1, le=25)):
    ...
    records = []
    for asset in ASSET_PROFILES_CACHE:
        if str(asset.get("asset_class")) != "transformer":
            continue
        asset_phase = (zlib.crc32(str(asset.get("asset_id")).encode()) % 628) / 100.0
        base_load = float(asset.get("base_load_pct") or 50.0)
        load_pct = min(132.0, max(20.0, base_load * (0.92 + 0.30 * math.sin(tick + asset_phase))))
    ...
    return {"timestamp": now.isoformat(), "source": "simulated real-time telemetry — not utility SCADA", "transformers": records[:limit]}
...
def sync_alerts():
    ...
    return {"source": "simulated telemetry — notifications are recorded locally only", "alerts": list(alerts.values())}
...
def search_cities(q: str = Query("", description="City or state name query"), limit: int = 15):
    ...
    q_low = q.lower()
    matches = [c for c in CITIES_CACHE if q_low in str(c.get("city", "")).lower() or q_low in str(c.get("state", "")).lower()]
